...
fn main() {
    ...
    let t1 = thread::spawn(move || map_data(&x1));
    let t2 = thread::spawn(move || map_data(&x2));
    ...
    for i in 0..num_partitions {
      let data = v2[i].clone();
      let data_thread = thread::spawn(move || map_data(&data));
      let _return = data_thread.join().unwrap();
      intermediate_sums_2.push(_return);
    }

    //println!("intermediate sums 2 length: {}", intermediate_sums_2.len());
    ...
    let sum2 = reduce_data(&intermediate_sums_2);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
  ...
  // will hold the data we send back
  let mut xs: Vec<Vec<usize>> = Vec::new();

  for i in 0..num_partitions {
    let x = partition_size*i;
    let y = (i+1)*partition_size;

    // make a vector for each partition
    let mut vector : Vec<usize> = Vec::new();

    // fill the partition with values
    for j in x..y {
      vector.push(v[j]);
      //println!("J: {}", j);
    }

    // push vector onto 2d vector
    xs.push(vector);
  }
