  ...
  for (int i = 0; i < fileSize; i++) {
    ...
    if (buffer[i] != ' ' && buffer[i] != '\0') {
      if (buffer[i] > 'Z' || buffer[i] < 'A') {
        fprintf(stderr, "%s", "dec_client error: input contains bad characters\n");
        exit(1);
      }
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  strcat(buffer, "d_"); // tells the server that we're decrypting (incase we use the wrong server)
  strcat(buffer, ciphertext);
  strcat(buffer, "&"); // put & inbetween the plaintext and the key
  strcat(buffer, key);
